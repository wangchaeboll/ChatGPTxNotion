...
class BlockEditor {
  ...
  createWrapper() {
    ...
    block.addEventListener("input", (e) => {
      const id = e.target.closest(".wrapper").dataset.id;
      const text = e.target.innerText.trim();

      clearTimeout(this.saveTimeout);

      setTimeout(() => {
        if (this.blockContents[id].mode !== "text") return;

        this.saveTimeout = setTimeout(() => {
          this.blockContents[id].content = text;
        }, 300);
      }, 0);
    });
  ...
  openMenu(e, blockId) {
    ...
    this.menu.innerHTML = `<div role="button" id="change" style=" padding: 6px 12px; background-color: #007bff; color: white; border: none;border-radius: 4px; cursor: pointer; font-size: 14px;">Equation Block</div>`;
    this.menu.style.left = `${rect.left}px`;
    this.menu.style.top = `${rect.bottom}px`;
    this.menu.style.width = `${rect.width - 8}px`;
    this.menu.style.display = "block";
    const change = document.querySelector("#change");
    change.addEventListener("click", (e) => this.EquationBlock(e, blockId));
  }
  ...
  EquationBlock(e, blockId) {
    ...
    equation.addEventListener("click", (e) => {
      ...
      this.menu.querySelector(".latbox").innerText = this.blockContents[blockId].content ? this.blockContents[blockId].content : "Start Typing";
    });

    // menu.addEventListener("input", () => {
    //   const id = this.currentBlock?.dataset?.id;
    //   const latex = menu.innerText.trim();

    //   clearTimeout(this.saveTimeout);
    //   this.saveTimeout = setTimeout(() => {
    //     // update internal storage
    //     this.blockContents[id] = {
    //       mode: "equation",
    //       content: latex,
    //     };

    //     // re-render equation block
    //     try {
    //       const renderArea = this.currentBlock.querySelector(".interior");
    //       this.renderMath(latex, renderArea);
    //     } catch (err) {
    //       console.warn("Invalid equation:", err);
    //     }
    //   }, 300);
    // });

    // equation
    //   .querySelector(".del-icon")
    //   .addEventListener("click", this.deleteEqBlock);
  }
