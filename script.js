...
function positionMenu(input) {
  ...
  menu.style.top =
    spaceBelow > 150 ? rect.bottom + "px" : rect.top - menu.offsetHeight + "px";
}
...
function blockKeyHandler(e) {
  ...
  if (e.key === "Enter" && e.shiftKey) {
    ...
    const state = this.parentElement.parentElement.querySelector("#menu");
    if (text === "/eq") {
...
function EquationBlock(e, state) {
  e.preventDefault();
  const wrapper = this.parentElement;
