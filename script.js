...
//   document.getElementById("renderBtn").addEventListener("click", renderMath);
// });
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  if (initialWrapper) {
    initialWrapper.addEventListener("mouseenter", blockHover);
    initialWrapper.addEventListener("mouseleave", blockUnhover);
    initialWrapper.querySelector(".copy-icon").addEventListener("click", copy);
  }
});
...
function createWrapper() {
  ...
  wrapper.addEventListener("mouseenter", blockHover);
  wrapper.addEventListener("mouseleave", blockUnhover);

  const block = document.createElement("div");
  block.className = "block";
  block.contentEditable = "true";
  block.addEventListener("keydown", blockKeyHandler);
...
function blockKeyHandler(e) {
  ...
  if (e.key === "Enter" && e.shiftKey) {
    ...
    if (lastArrowKey === "ArrowDown" && now - lastArrowTime < 500) {
    ...
    if (text === "/eq") {
      EquationBlock.call(this, e, state);
      const newWrapper = createWrapper();
      wrapper.insertAdjacentElement("afterend", newWrapper);
      newWrapper.querySelector(".block").focus();
      return;
...
function EquationBlock(e) {
  ...
  equation.addEventListener("click", (e) => {
    ...
    const eqText = e.currentTarget.querySelector(".interior").getBoundingClientRect()

    menu.style.left = `${rect.left + window.scrollX}px`;
    menu.style.top = `${rect.bottom + window.scrollY}px`;
    menu.style.width = `${eqText.width}px`;
    menu.style.display = "block";
  });
  equation.querySelector(".del-icon").addEventListener("click", deleteEqBlock);
}

function blockHover(e) {
  const svg = this.querySelector(".copy-icon svg");
  const del = this.querySelector(".del-icon");
  if (svg) {
    svg.style.opacity = "1";
